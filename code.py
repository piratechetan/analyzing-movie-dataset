...
def explore_data(dataset, start, end, rows_and_columns=False):
    ...
    """
    data_slicing = dataset[start:end]
    for row in dataset:
        print(row)
        print("\n")        
    if rows_and_columns:
        print("Numbers ofrows",len(dataset))
...
def duplicate_and_unique_movies(dataset, index_):
    ...
    for movie in dataset:
        ...
        if movie in unique:
            ...
            unique.append(name)  
        print("Numbers of duplicate movies",len(duplicate))
        print("\n")          
...
# Our dataset might have more thanvies one entry for a movie. Call duplicate_and_unique_movies() with index of the name to check the same.
